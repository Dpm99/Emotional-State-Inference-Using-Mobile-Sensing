...
def preprocessing(uid):
    ...
    df_call_raw = pd.read_csv('/home/diogo_mota/Dropbox/QMUL/MSc_Project/dataset/call_log/call_log_'+uid+'.csv')
    #df_call_raw
    ...
    df_call_clean = df_call_raw.drop(['id', 'device', 'CALLS_name', 'CALLS_number',
                                 'CALLS_numberlabel', 'CALLS_numbertype'], axis = 1)
    df_call_clean['date'] = df_call_clean['timestamp'].apply(lambda x: time.strftime("%Y-%m-%d",time.gmtime(x)))


    # In[16]:


    df_call = df_call_clean[['date', 'CALLS_duration']]

    if len(df_call.columns[df_call.isna().any()].tolist())!=0:
        for name in df_call.columns[df_call.isna().any()].tolist():
            df_call[name] = data_imputer(df_call, name)
    else:
        pass
    ...
    df_call_mean = df_call.groupby('date').mean()
    df_call_mean.columns = ['Mean_call_duration']

    df_call_std = df_call.groupby('date').std(ddof=0)
    df_call_std.columns = ['Std_call_duration']

    df_call_median = df_call.groupby('date').median()
    df_call_median.columns = ['Median_call_duration']

    df_call_min = df_call.groupby('date').min()
    df_call_min.columns = ['Min_call_duration']

    df_call_max = df_call.groupby('date').max()
    df_call_max.columns = ['Max_call_duration']

    df_call_skew = df_call.groupby('date').skew()
    df_call_skew.columns = ['Skew_call_duration']

    df_call_var = df_call.groupby('date').var(ddof=0)
    df_call_var.columns = ['Var_call_duration']


    # In[18]:


    data_call = [df_call_mean["Mean_call_duration"], df_call_std["Std_call_duration"],
                 df_call_median['Median_call_duration'], df_call_min['Min_call_duration'], 
            df_call_max['Max_call_duration'], df_call_skew['Skew_call_duration'], df_call_var['Var_call_duration']]

    headers_call = ['Mean_Call_Duration', 'Std_Call_Duration','Median_Call_Duration', 'Min_Call_Duration', 
               'Max_Call_Duration',
              'Skew_Call_Duration', 'Var_Call_Duration']

    df_call_data = pd.concat(data_call, axis=1, keys=headers_call)
    df_call_data = final_check(df_call_data)
    #df_call_data
    ...
    df_sms_raw = pd.read_csv('/home/diogo_mota/Dropbox/QMUL/MSc_Project/dataset/sms/sms_'+uid+'.csv')
    #df_sms_raw
    ...
    df_sms_raw['date'] = df_sms_raw['timestamp'].apply(lambda x: time.strftime("%Y-%m-%d",time.gmtime(x)))
    #df_sms_raw['time'] = df_sms_raw['timestamp'].apply(lambda x: time.strftime("%H:%M:%S",time.gmtime(x)))


    # In[76]:


    df_sms = df_sms_raw[['date','MESSAGES_read', 'MESSAGES_reply_path_present', 'MESSAGES_type']]
    #df_sms
    ...
    unique_date = df_sms['date'].unique()
    key_list = []
    number_sms_list = []
    for val in unique_date:
        
        number_sms = len(df_sms[df_sms['date']==val])
        key_list.append(val)
        number_sms_list.append(number_sms)


    # In[78]:


    df_sms_data = pd.DataFrame(number_sms_list, columns = ['Number of sms'], index=key_list)
    df_sms_data.index.name = 'date'
    #df_sms_data
    ...
    student_df = pd.merge(df_call_data, df_activity_data, left_index=True, right_index=True)

    df_list = [df_audio_data, df_conv_data, df_dark_data, df_gps_data,
              df_phone_charge_data, df_phone_lock_data, df_wifi_loc_data,
              df_sms_data, pam_ema_data, sleep_ema_data, social_ema_data, stress_ema_data]
